					...
					<div class="table_div">
						<table style="width:1000px" align="center">
							...
							<tr>
								...
								<td>${dto.orDate}</td>
								<td>
									<input type="button" value="승인" onclick="window.location='productOrderApproval.st?orNo=${dto.orNo}&pageNum=${paging.pageNum}'">
								</td>
								<td>
									<input type="button" value="취소" onclick="window.location='productOrderCancel.st?orNo=${dto.orNo}&pageNum=${paging.pageNum}'">
								</td>
							</tr>
							</c:forEach>
							<!-- 페이징 -->
							<tr>
								<td colspan="12" align="center" id="testTest">
									...
									<c:if test="${paging.startPage > 10}">
										<a href="${path}/productOrderList.st?pageNum=${paging.prev}">[이전]</a>
									</c:if>
									
									<!-- 페이지번호 처리 -->
									<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
										<a href="${path}/productOrderList.st?pageNum=${num}">${num}</a>
									</c:forEach>
									
									<!-- 다음버튼 활성화 여부 -->
									<c:if test="${paging.endPage < paging.pageCount}" >
										<a href="${path}/productOrderList.st?pageNum=${paging.next}">[다음]</a>
									</c:if>
