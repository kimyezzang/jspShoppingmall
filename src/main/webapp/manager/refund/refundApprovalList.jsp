							...
							<tr>
								<td colspan="12" align="center" id="testTest">
									...
									<c:if test="${paging.startPage > 10}">
										<a href="${path}/refundApprovalList.st?pageNum=${paging.prev}">[이전]</a>
									</c:if>
									
									<!-- 페이지번호 처리 -->
									<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
										<a href="${path}/refundApprovalList.st?pageNum=${num}">${num}</a>
									</c:forEach>
									
									<!-- 다음버튼 활성화 여부 -->
									<c:if test="${paging.endPage < paging.pageCount}" >
										<a href="${path}/refundApprovalList.st?pageNum=${paging.next}">[다음]</a>
									</c:if>
