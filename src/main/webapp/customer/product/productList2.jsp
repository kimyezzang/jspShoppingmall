...
<div id="container" style="height:1200px;" >
   ...
   <div class="product_category"><h2 >하의</h2>
   </div>
   <!-- 게시글이 있으면 -->
		...
		<tr>
			<td colspan="11" align="center" id="testTest" style="margin-top:200px; padding-top:50px;">
				...
				<c:if test="${paging.startPage > 10}">
					<a href="${path}/productList2.do?pageNum=${paging.prev}">[이전]</a>
				</c:if>
				
				<!-- 페이지번호 처리 -->
				<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
					<a href="${path}/productList2.do?pageNum=${num}">${num}</a>
				</c:forEach>
				
				<!-- 다음버튼 활성화 여부 -->
				<c:if test="${paging.endPage < paging.pageCount}" >
					<a href="${path}/productList2.do?pageNum=${paging.next}">[다음]</a>
				</c:if>
